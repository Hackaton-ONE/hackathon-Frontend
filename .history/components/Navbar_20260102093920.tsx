  ...
  return (
    ...
    <div className="relative top-0 left-0 w-full z-50 px-4 py-8">
      ...
      <div className="max-w-[1600px] mx-auto flex justify-between items-center lg:grid lg:grid-cols-3">
        ...
        {/* =======================
            2. PÍLULA CENTRAL (Coluna 2)
            MUDANÇA: 'hidden lg:flex' 
            (Escondido em Tablet/Mobile, Flex em Notebook+)
           ======================= */}
      ...
      {/* =======================
         ...
         ======================= */}
      {isOpen && (
        <div className="lg:hidden absolute top-24 left-4 right-4 z-40 bg-mood-dark/95 backdrop-blur-xl border border-white/10 rounded-2xl p-4 shadow-2xl animate-in fade-in slide-in-from-top-5">
          
          <div className="flex flex-col space-y-1">
            <Link href="/" onClick={() => setIsOpen(false)} className="block px-4 py-3 text-white/90 hover:text-white hover:bg-blue-default rounded-lg transition-colors text-sm font-medium">
              Início
            </Link>
          ...
          <Link href="/analisar" onClick={() => setIsOpen(false)}>
            <button className="w-full bg-white hover:bg-neon-orange text-black border border-white/20 font-poppins font-bold text-sm py-3 rounded-lg transition-all duration-200">
              Analisar
            </button>
          </Link>
